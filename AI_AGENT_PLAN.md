...
## Overview
This document outlines the complete plan for integrating an AI agent (Claude Sonnet 4.5) that can understand canvas structures, answer questions, and add nodes/edges based on user requests.
...
src/services/ai/
  ...
  ├── contextExtractor.ts       # Canvas → text serialization
  ...
  ├── nodeGenerator.ts          # Convert AI responses to React Flow format
...
src/components/canvas/
  └── NotesSidebar.tsx          # Enhanced with token warnings & context display
```
...
**Tasks:**
- [x] Create function to serialize nodes to text format
- [x] Create function to serialize edges to text format
- [x] Add graph analysis (central nodes, clusters, isolated nodes)
- [x] Create `extractCanvasContext(nodes, edges, options)` function
- [x] Support options:
  - `includePositions`: boolean (default: true)
  - `includeTags`: boolean (default: true)
  - `maxNodes`: number (for truncation)
  - `selectedNodeIds`: string[] (prioritize selected nodes)
  - `includeGraphAnalysis`: boolean (default: true)
- [x] Format output as structured text for AI consumption

**Output Format:**
```
CANVAS STRUCTURE:
- Nodes (N nodes):
  * Node 1 [ID: "1", Type: rectangle, Label: "User Authentication", Tags: ["Idea", "New"], Position: (100, 200)]
  * Node 2 [ID: "2", Type: circle, Label: "Database", Tags: ["Thinking"], Position: (300, 200)]
  
- Edges (E edges):
  * Edge 1: "User Authentication" → "Database" [Label: "stores data in"]
  
- Graph Analysis:
  * Central nodes: Node 1 (connected to 3 nodes)
  * Isolated nodes: None
  * Clusters: 1 main cluster
```

**Acceptance Criteria:**
- ✅ Produces clear, structured text representation
- ✅ Handles empty canvas gracefully
- ✅ Supports all node types (rectangle, circle, diamond, triangle)
- ✅ Includes all edge information (source, target, labels)
...
**Tasks:**
- [x] Create system prompt template with:
  - Role definition (AI assistant for knowledge graphs)
  - Capabilities (understand, answer, add, clarify)
  - Canvas format explanation
  - Response format requirements
- [x] Create `buildPrompt(context, messages, intent)` function
- [x] Support dynamic prompt construction based on intent
- [x] Include conversation history (last 10-15 messages, configurable)
- [x] Add canvas context with proper formatting
- [x] Inject user's current message
- [x] Add intent-specific instructions

**System Prompt Structure:**
```
You are an AI assistant helping users explore and build knowledge graphs on a canvas.
...
CANVAS FORMAT:
- Nodes have: ID, type (rectangle/circle/diamond/triangle), label, position, tags
...
RESPONSE FORMAT:
For ADD operations: Return JSON with nodes and edges to add
For QUERY operations: Return natural language explanation
...
### Phase 2.4: Node/Edge Generator
**Dependencies:** Phase 2.2  
**Files:** `src/services/ai/nodeGenerator.ts`  
**Status:** ✅ Completed

**Tasks:**
- [x] Create `generateNodes(nodeData: NodeData[], options)` function
- [x] Create `generateEdges(edgeData: EdgeData[], ...)` function
- [x] Handle position calculation:
  - Absolute positions (if provided)
  - Relative positions (calculate from reference node)
  - Smart defaults (if no position specified)
- [x] Resolve node references (label → ID mapping)
- [x] Generate unique node IDs (integrate with nodeIdCounter)
- [x] Validate node/edge data before generation
- [x] Handle edge cases:
...
- Relative position: calculates from reference node (existing or new)
- Smart default: positions near center of existing nodes with random offset
- Fallback: random position in visible area
- Conflict detection: checks for overlaps and adjusts position

**Acceptance Criteria:**
- ✅ Generates valid React Flow nodes/edges
- ✅ Calculates positions correctly
- ✅ Resolves node references accurately (by ID or label)
...
**Files:** `src/hooks/useChat.ts`, `src/components/CanvasPage.tsx`, `src/components/canvas/NotesSidebar.tsx`  
**Status:** ✅ Completed

**Tasks:**
- [x] Add AI response generation after user messages
- [x] Integrate context manager for token tracking
- [x] Call Claude API with proper prompt
- [x] Parse AI response
- [x] Handle different response types:
  - ADD → Generate nodes/edges (return for parent to apply)
  - ANSWER → Save as assistant message
  - CLARIFY → Save as assistant message with clarification state
- [x] Add loading states for AI processing
- [x] Handle errors gracefully
- [x] Update message state with AI responses
...
- ✅ Context warnings displayed (passed to NotesSidebar)
...
**Tasks:**
- [x] Connect AI-generated nodes/edges to canvas state
- [x] Handle ADD action responses:
  - Extract nodes/edges from AI response
  - Add to React Flow state using existing functions
  - Update nodeIdCounter
  - Trigger auto-save (via existing auto-save mechanism)
- [x] Handle errors (invalid nodes, position conflicts)
- [x] Save AI explanation as assistant message

**Integration Points:**
- ✅ Uses existing `setNodes` and `setEdges` from React Flow
- ✅ Integrates with `nodeIdCounter` state
- ✅ Uses existing auto-save mechanism (triggers automatically on state change)
- ✅ Uses `generateNodesAndEdges()` from nodeGenerator service

**Acceptance Criteria:**
- ✅ AI-generated nodes appear on canvas
- ✅ Nodes positioned correctly (with conflict detection)
...
**Tasks:**
- [x] Create graph analysis utilities:
  - Find paths between nodes (BFS for shortest path, DFS for all paths)
  - Identify central nodes (degree, betweenness, closeness centrality)
  - Detect clusters (connected components with density metrics)
  - Find isolated nodes
  - Bridge node detection
  - Relationship strength calculation
- [x] Enhance context extractor with relationship analysis
- [x] Improve AI prompts with relationship insights
- [x] Better answers for relationship queries

**Analysis Functions:**
```typescript
findPath(sourceId, targetId, nodes, edges): PathResult | null
findAllPaths(sourceId, targetId, nodes, edges, maxDepth): PathResult[]
getCentralNodes(nodes, edges, topN): CentralityMetrics[]
detectClusters(nodes, edges): Cluster[]
getIsolatedNodes(nodes, edges): Node[]
analyzeRelationship(sourceId, targetId, nodes, edges): RelationshipInsight
findBridgeNodes(nodes, edges): string[]
getRelationshipAnalysis(nodes, edges): ComprehensiveAnalysis
```

**Acceptance Criteria:**
- ✅ Accurate relationship analysis (BFS/DFS algorithms)
- ✅ Fast computation (O(V+E) for BFS, efficient for typical graphs)
